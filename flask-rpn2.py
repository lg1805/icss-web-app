...
def upload_file():
    ...
    if file:
        ...
        with pd.ExcelWriter(processed_filepath, engine='xlsxwriter') as writer:
            ...
            worksheet_spn = writer.sheets["SPN"]
            
            # Apply formatting to Incident Status column
            status_col_idx = spn_df.columns.get_loc("Incident Status")
            for row_num, value in enumerate(spn_df["Incident Status"], start=1):
                if str(value).strip().lower() in ['closed', 'completed']:
                    worksheet_spn.write_string(row_num + 1, status_col_idx, str(value), green_format)
            
            # Write Non-SPN sheet
            non_spn_df.to_excel(writer, sheet_name="Non-SPN", index=False)
            worksheet_non_spn = writer.sheets["Non-SPN"]
            
            # Apply formatting to Incident Status column
            for row_num, value in enumerate(non_spn_df["Incident Status"], start=1):
                if str(value).strip().lower() in ['closed', 'completed']:
                    worksheet_non_spn.write_string(row_num + 1, status_col_idx, str(value), green_format)
